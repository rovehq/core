...
impl DaemonManager {
    pub(super) fn verify_manifest_at_startup() -> std::result::Result<(), String> {
        ...
        let crypto = CryptoModule::new()
            .map_err(|error| format!("Failed to initialize crypto: {}", error))?;
        ...
        if let Some(signature) = manifest.get("signature").and_then(|value| value.as_str()) {
            ...
            let verify_bytes = serde_json::to_vec(&manifest_for_verify).map_err(|error| {
                format!("Failed to serialize manifest for verification: {}", error)
            })?;
