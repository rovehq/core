...
pub fn keychain_get(key: &str) -> Result<String, EngineError> {
    ...
    let output = Command::new("security")
        .args(["find-generic-password", "-a", &username, "-s", key, "-w"])
        .output()
        .map_err(|e| {
            EngineError::KeyringError(format!("Failed to execute security command: {}", e))
        })?;
...
pub fn keychain_set(key: &str, value: &str) -> Result<(), EngineError> {
    ...
    let _ = Command::new("security")
        .args(["delete-generic-password", "-a", &username, "-s", key])
        .output();
