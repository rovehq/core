...
async fn setup_test_db() -> Result<SqlitePool> {
    ...
    // Create episodic_memory table with all Phase 2 fields
    sqlx::query(
    ...
    // Create episodic_fts with domain field
    sqlx::query(
        "CREATE VIRTUAL TABLE episodic_fts USING fts5(
            summary, entities, topics, tags, domain,
    ...
    // Create triggers for episodic_fts
    sqlx::query(
        "CREATE TRIGGER episodic_fts_ai AFTER INSERT ON episodic_memory BEGIN
            INSERT INTO episodic_fts(rowid, summary, entities, topics, tags, domain)
            VALUES (new.rowid, new.summary, new.entities, new.topics, '', new.domain);
        END;",
    )
    .execute(&pool)
    .await?;
    ...
    sqlx::query(
        "CREATE TRIGGER episodic_fts_au AFTER UPDATE ON episodic_memory BEGIN
            INSERT INTO episodic_fts(episodic_fts, rowid, summary, entities, topics, tags, domain)
            VALUES ('delete', old.rowid, old.summary, old.entities, old.topics, '', old.domain);
            INSERT INTO episodic_fts(rowid, summary, entities, topics, tags, domain)
            VALUES (new.rowid, new.summary, new.entities, new.topics, '', new.domain);
        END;",
    )
    ...
    sqlx::query(
        "CREATE TRIGGER episodic_fts_ad AFTER DELETE ON episodic_memory BEGIN
            INSERT INTO episodic_fts(episodic_fts, rowid, summary, entities, topics, tags, domain)
            VALUES ('delete', old.rowid, old.summary, old.entities, old.topics, '', old.domain);
        END;",
    )
    ...
    // Create consolidation_insights table with domain field
    sqlx::query(
        "CREATE TABLE consolidation_insights (
            id TEXT PRIMARY KEY,
            created_at INTEGER NOT NULL,
    ...
    // Create insights_fts with domain field
    sqlx::query(
        "CREATE VIRTUAL TABLE insights_fts USING fts5(
            insight, domain,
            content='consolidation_insights', content_rowid='rowid'
        );",
    )
    .execute(&pool)
    .await?;

    // Create triggers for insights_fts
    sqlx::query(
        "CREATE TRIGGER insights_fts_ai AFTER INSERT ON consolidation_insights BEGIN
    ...
    sqlx::query(
        "CREATE TRIGGER insights_fts_au AFTER UPDATE ON consolidation_insights BEGIN
    ...
    sqlx::query(
        "CREATE TRIGGER insights_fts_ad AFTER DELETE ON consolidation_insights BEGIN
            INSERT INTO insights_fts(insights_fts, rowid, insight, domain)
            VALUES ('delete', old.rowid, old.insight, old.domain);
        END;",
    )
