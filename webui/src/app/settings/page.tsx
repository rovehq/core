...
export default function SettingsPage() {
  const { appState, authStatus, hello, initialize, lock } = useRoveStore();
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-6 space-y-6">
        <section className="bg-surface rounded-xl p-6 border border-surface2">
          ...
          <div className="space-y-6">
            ...
            <div>
              <h3 className="font-medium mb-4">Timeouts</h3>
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 bg-surface2 rounded-lg">
                  <div>
                    <p className="font-medium">Idle unlock window</p>
                    <p className="text-sm text-gray-500">
                      {formatSeconds(authStatus?.idle_expires_in_secs)}
                    </p>
                  </div>
                </div>
                <div className="flex items-center justify-between p-4 bg-surface2 rounded-lg">
                  <div>
                    <p className="font-medium">Absolute session window</p>
                    <p className="text-sm text-gray-500">
                      {formatSeconds(authStatus?.absolute_expires_in_secs)}
                    </p>
                  </div>
                </div>
